...
- [~] No live project exists yet — everything below is scaffolded and locally verified (schema syntax, TypeScript types, real bundling) but **nothing has run against an actual Supabase instance**. To go live: create a project, run `supabase link` + `supabase db push` from `supabase/` at the repo root, configure the Apple OAuth provider in the dashboard, and fill in `app/.env.local` from `app/.env.example` (using the **publishable key**, not the legacy anon key — see the note below).
