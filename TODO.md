...
- [~] **P0** Google sign-in — **code is real, wiring is done, the one remaining step is a free external account.** `@react-native-google-signin/google-signin` installed, `auth.tsx`'s `signInWithGoogle()` mirrors the Apple pattern exactly (`GoogleSignin.signIn()` → `supabase.auth.signInWithIdToken({ provider: 'google', token })`), and a "Continue with Google" button is live on `(onboarding)/auth.tsx`. Deliberately **not using the library's default Expo config plugin** — its no-argument default assumes Firebase and requires a `google-services.json` this project doesn't have; the plugin entry was removed from `app.json` entirely rather than fighting its Firebase-only default, since plain OAuth (no Firebase) needs no plugin-applied native config at all, just autolinking. Reads `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` from env (`.env.example` documents it) and fails with an honest "not configured yet" message when unset, rather than crashing. **What's left**: create a free Google Cloud OAuth client (web + Android, no Play Console needed) and register this app's SHA-1 — an EAS-managed keystore now exists (see below) to get that fingerprint from.
- [x] **P0** EAS project + dev-client build pipeline — `expo-dev-client` installed, EAS project created and linked (`@kimkemp/lockedin`, `app.json`'s `extra.eas.projectId`), `eas.json` added with development/preview/production Android profiles, and a real cloud keystore generated (EAS-managed signing credentials, no local `keytool` needed). **First Android development build succeeded** (build `39bb72fa`, ~22 min, real installable APK: `https://expo.dev/artifacts/eas/9g6U5m5eGLuD1urAEowCdmEbdDL9YAzycuoZ5YLvbKk.apk`) — a real, working proof that the native module pipeline (dev-client + google-signin autolinking without Firebase) actually compiles, not just "should work." The first attempt failed in the "Install dependencies" phase on the same ERESOLVE conflict §0.3 already documented for plain `npm ci`; fixed with a committed `.npmrc` (`legacy-peer-deps=true`) so EAS Build's install step — which has no way to pass a manual flag — resolves the same tree the lockfile expects. This is the one piece of "needs a dev client" work that had zero external-account blocker: no Play Console account was needed to reach a working build, confirming the §17 intro's claim that Android's native-feature blocker is genuinely softer than iOS's. **Not yet done**: installing this APK on a real device and confirming "Continue with Google" actually opens the native flow (still needs the Google Cloud OAuth client to complete sign-in, but the build/install/launch path itself is now unblocked).
...
## Phase 18 — Marketing Site: Waitlist (lockedinmission.app)

*Gate: a real visitor can submit their email on a real device and the row lands in Supabase.*

**Added 2026-09-12, live the same day.** ⚠️ **Domain name correction**: every reference up to and including the initial build (this repo's `marketing/lockedinmissions/` directory name, the original migration's `source` default, earlier commit messages) said "lockedinmissions.app" (plural). Actually connecting to cPanel revealed that's wrong — DNS doesn't resolve it at all. The real, live, registered domain, confirmed by both DNS and the account's own cPanel documentation, is **`lockedinmission.app`** (singular). Fixed where it matters (the database default, this section) via a follow-up migration rather than editing the already-applied one; the `marketing/lockedinmissions/` folder name itself is repo-internal bookkeeping and wasn't renamed, but don't read anything into that spelling.

- [x] **P0** Landing page built — `marketing/lockedinmissions/{index.html,style.css,script.js}`, plain static HTML/CSS/vanilla JS (no build step, no bundler), matching shared cPanel hosting's constraints. Sourced directly from the app's own real design tokens (`app/src/theme/colors.ts`/`type.ts` — Void/Obsidian/Electric/Violet/Iris/Gold, Outfit/Inter/JetBrains Mono), not a separate palette invented for the marketing site. Structurally referenced `forfeit.app` (hero → how-it-works → features → CTA → FAQ funnel shape) but with **no fabricated social proof** — no invented metrics or testimonials, since LockedIn has no public users yet; an honest "building in public" framing stands in where Forfeit shows real numbers. Content is the real, already-shipped feature set (AI-verified proof, Screenshot Intelligence's "never stored" privacy design, XP/streaks, Recovery Mode's no-shame framing) and the real 5-step core loop, not invented copy. Footer links to the already-hosted Privacy Policy/Terms (same artifact linked from the app's own Profile screen).
- [x] **P0** Waitlist backend — `supabase/migrations/20260912085808_waitlist_signups.sql` + `20260912103000_fix_waitlist_domain.sql`. A deliberate, narrow exception to this schema's otherwise-universal "every write needs a real authenticated user" rule: `waitlist_signups` (email unique, optional platform interest, source, created_at) with **exactly one RLS policy** — anonymous `insert`, no `select` for anyone but service_role, so the public page's embedded key can add an email but never read the list back out even if the page source were scraped. Live-verified end to end against the real project, twice: once pre-deployment (anon insert → 201, anon select → empty) and once again as a genuine end-to-end test against the live deployed page's own domain after upload.
- [x] **P1** Live preview published for review: https://claude.ai/code/artifact/9b969a94-9f43-4d5b-8ef3-b4ee6da945e6 — a derived copy of the same HTML/CSS/JS (adapted only for the Artifact tool's page-content-only wrapper requirement, not a different design).
- [x] **P0** Deployed to `lockedinmission.app` via SFTP (credentials in the gitignored `CPANEL.md`, same handling as `claudeapi.md`) — `public_html/` was the account's actual document root (a single-domain cPanel account, no addon-domain subfolder). All five files (`index.html`, `style.css`, `script.js`, `assets/{icon,favicon}.png`) confirmed live with `200`s at their real URLs, and a real signup was submitted against the live page's own domain and confirmed to land in `waitlist_signups` before being cleaned up.
