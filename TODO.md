...
## Phase 18 — Marketing Site: Waitlist (lockedinmissions.app)

*Gate: a real visitor can submit their email on a real device and the row lands in Supabase.*

**Added 2026-09-12.** `lockedinmissions.app` is live (hosting exists; cPanel credentials to follow — actual upload is a manual step once shared, not done yet). With the Google Play developer account still being set up, there's nothing to download yet, so this is a pre-launch waitlist page, not an app-store landing page.
...
- [x] **P0** Waitlist backend — `supabase/migrations/20260912085808_waitlist_signups.sql`. A deliberate, narrow exception to this schema's otherwise-universal "every write needs a real authenticated user" rule: `waitlist_signups` (email unique, optional platform interest, source, created_at) with **exactly one RLS policy** — anonymous `insert`, no `select` for anyone but service_role, so the public page's embedded key can add an email but never read the list back out even if the page source were scraped. Live-verified end to end against the real project: an anon-key insert returns 201 and the row is queryable via service_role; an anon-key select returns an empty result. Reuses the **same live Supabase project** already backing the app — one list, not a second system.
- [x] **P1** Live preview published for review before cPanel access exists: https://claude.ai/code/artifact/9b969a94-9f43-4d5b-8ef3-b4ee6da945e6 — a derived copy of the same HTML/CSS/JS (adapted only for the Artifact tool's page-content-only wrapper requirement, not a different design).
- [ ] **P0** Deploy to `lockedinmissions.app` — blocked on cPanel credentials, not yet provided. Upload `marketing/lockedinmissions/*` (including `assets/`) to the domain's document root via File Manager or FTP once available.
