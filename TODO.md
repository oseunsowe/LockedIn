...
- [x] **P0** EAS project + dev-client build pipeline — `expo-dev-client` installed, EAS project created and linked (`@kimkemp/lockedin`, `app.json`'s `extra.eas.projectId`), `eas.json` added with development/preview/production Android profiles, and a real cloud keystore generated (EAS-managed signing credentials, no local `keytool` needed). First Android development build kicked off from this environment — see the build link in the session log. This is the one piece of "needs a dev client" work that had zero external-account blocker: no Play Console account was needed to reach this point, confirming the §17 intro's claim that Android's native-feature blocker is genuinely softer than iOS's.
